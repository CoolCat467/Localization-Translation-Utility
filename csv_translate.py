...
async def translate_file(sentances: list[str],
                         client: httpx.AsyncClient,
                         to_lang: str,
                         src_lang: str = 'auto') -> list[str]:
    ...
    results = await translate.translate_async(client, modified,
                                              to_lang, src_lang)
...
def write_nanosaur_localization(filename: str,
                                languages: dict[str, list[str]]) -> None:
...
async def create_translation(languages: dict[str, list[str]],
                             code: str, client: httpx.AsyncClient) -> None:
...
async def async_run() -> None:
    ...
    async with httpx.AsyncClient(http2 = True) as client:
        contents = await download_file('Data/System/strings.csv',
                                       cache, client)
