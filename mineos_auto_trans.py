...
async def abstract_translate(
    ...
    print("\nGettting files...")
    files, file_comments = await download_lang(
        "Installer/Files.cfg", cache_folder, client
    )
    # Get true copy
    ...
    for section in search:
        ...
        for folder, exists in section_to_walk(lang_files):
            ...
            english, comments = await download_lang(
                f"{folder}/English.lang", cache_folder, client
            )
            ...
            new_files += await translate_file_given_coro(
                trans_coro, (english, comments), lang_data, folder
            )
    ...
    if files == orig_files:
        ...
        file_comments = convert.update_comment_positions(
            file_comments, files, orig_files
        )
