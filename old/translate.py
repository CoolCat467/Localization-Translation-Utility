#!/usr/bin/env python3
# Tiny translator module using sneeky tricks from google's dictionary chrome extension
# -*- coding: utf-8 -*-
...
def translate_sync(
    sentence: str, to_lang: str, source_lang: str = "auto"
) -> str:
...
async def get_translated_coroutine(
    loop, sentence: str, to_lang: str, source_lang="auto"
) -> str:
    ...
    # the google translate dictionary extension running in google chrome.
    async with aiohttp.ClientSession(
        loop=loop, headers=MAGIC_HEADER
    ) as session:
...
async def translate_async(
    loop, sentences: list, to_lang: str, source_lang: str = "auto"
) -> list:
    "Translate multiple sentences asynchronously."
    # Get a bunch of tasks running at once...
    coros = [
        get_translated_coroutine(loop, q, to_lang, source_lang)
        for q in sentences
    ]
...
def translate_sentances(
    sentences: list, to_lang: str, source_lang: str = "auto"
) -> list:
    "Translate many sentences at once using the power of asynchronous code."
    # Get asynchronous event loop
    event_loop = asyncio.new_event_loop()
    # Run the main function until it's done and get our translated sentences
    data = event_loop.run_until_complete(
        translate_async(event_loop, sentences, to_lang, source_lang)
    )
    # Close event loop
    event_loop.close()
...
def run() -> None:
    ...
    def timed(func) -> Callable[..., Any]:
        ...
        def time_func(*args, **kwargs) -> Any:
            ...
            print(f"\n{func.__name__} took {stop-start:.4f} seconds.\n")
            return result

        return time_func

    # Get a bunch of sentences to translate
    sentences = [
        "Hello",
        "Goodbye",
        "Cat",
        ...
        "Eggs",
        "Spam",
        "I can has cheeseburgers please",
        "Over the rainbow",
        "Muffin time",
        "Lolcat is too a language!",
        "This is a test",
        "Asynchronous code is faster hazah",
    ]
    # Tell the person watching us what they are
    print("We will be translating the following sentences...\n")
    print("\n".join(sentences) + "\n")
    ...
    print(
        f'... to the language corresponding with language code "{dst_lang}".'
    )
    print("\nlettus begin.\n")

    # Define two functions, one that translates our sentences one at a time (normally)
    @timed
    def without_async() -> None:
        print(
            "\n".join(
                translate_sync(sentence, dst_lang, src_lang)
                for sentence in sentences
            )
        )

    # and another that does it all asynchronously, both timed.
    @timed
    def with_async() -> None:
        print("\n".join(translate_sentances(sentences, dst_lang, src_lang)))

    # Translate the sentences with both to prove asynchronous is better woo

    if (
        input(
            "\nWould you like a demonstration of the translate module? (y/N) : "
        ).lower()
        != "y"
    ):
        return
